...
void main(){ 
	...
	vec4 diffuse_material = texture2D(dif, uvTexCoord);
	vec3 normal = texture2D(norm, uvTexCoord).rgb;
	vec4 specular_material = texture2D(spec, uvTexCoord);
	vec4 ambient_material = texture2D(amb, uvTexCoord);
	vec4 emit_material = texture2D(emit, uvTexCoord);
