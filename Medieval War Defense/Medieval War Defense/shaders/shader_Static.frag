...
void main(){ 
	...
	// converte espaco das coordenadas de textura pro mesmo espaco da posicao em que o objeto foi transformado
	vec2 objSpaceTexCoord = vec2((TexCoord * 2.0) - 1.0); 
	// transforma as coordenadas de textura neste espaco para equivalerem as coordenadas do objeto
	vec2 objTexCoord = (projection * view * model * vec4(objSpaceTexCoord, vec2(1.0f))).xy;
	// "desconverte" as coordenadas de textura para o espaco original, uma vez feita a transformacao
...
vec4 lastColor = vec4(0.0);
	...
	normal -= 0.5; // transformamos o intervalo de [0,1] para [-0.5,0.5]. Pois cada pixel representa uma direcao, nao cor.
	...
	for(int i = 0; i < totalLights; i++)
	{
		...
		// calculamos um vetor de distancia entre o pixel da textura e a posicao da luz;
		vec3 N = normalize(normal); // normalizamos esse vetor, pois so precisamos de sua direcao.

		//Calculo da atenuacao
		float D = length(L);
		float Attenuation = 1.0 / ( LightSource[i].falloff.x + (LightSource[i].falloff.y*D) + (LightSource[i].falloff.z*D*D) );

		// normalizamos o vetor que corresponde a essa distancia.
		L = normalize(L);
